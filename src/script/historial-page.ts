...
form?.addEventListener("submit", async (e) => {
  ...
  // const categoriaProdcutos = Object.keys(data);
  // limpiar resultados anteriores
  lista.innerHTML = "";

  
  let cardToggle = false
  nombresCategorias.forEach((item:any) => {
    ...
    card.innerHTML = `
       ...
       <h3 class="text-lg font-semibold text-center text-gray-800">${item}<h3/>
    ...
    card.addEventListener("click",() => {
      ...
      for(let itemElement of data){
        if(itemElement.productos.nombre === item ){
          cardToggle = !cardToggle
          

            card.innerHTML += `
            <div
            class="bg-white rounded-2xl shadow-md p-4 border border-gray-200">
              
            <h3 class="text-lg font-semibold text-center text-gray-800">${itemElement.productos.nombre}<h3/>
            <spam>fecha <spam/>
            ...
            <div>
              ...
              <select
                ...
                disabled
              >
                <option value="1" ${itemElement.estado_id == 1 ? "selected" : ""}>Inicio</option>
                <option value="2" ${itemElement.estado_id == 2 ? "selected" : ""}>Final</option>
                <option value="3" ${itemElement.estado_id == 3 ? "selected" : ""}>Ingreso</option>
                <option value="4" ${itemElement.estado_id == 4 ? "selected" : ""}>Envío</option>
                <option value="5" ${itemElement.estado_id == 5 ? "selected" : ""}>Devolución</option>
                <option value="6" ${itemElement.estado_id == 6 ? "selected" : ""}>Producción</option>
              </select>
            </div>
